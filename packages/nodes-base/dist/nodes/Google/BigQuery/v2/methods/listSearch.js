...
async function searchProjects(filter, paginationToken) {
    ...
    const response = await transport_1.googleBigQueryApiRequest.call(this, 'GET', '/v2/projects', undefined, qs);
...
async function searchDatasets(filter, paginationToken) {
    ...
    const response = await transport_1.googleBigQueryApiRequest.call(this, 'GET', `/v2/projects/${projectId}/datasets`, undefined, qs);
...
async function searchTables(filter, paginationToken) {
    ...
    const response = await transport_1.googleBigQueryApiRequest.call(this, 'GET', `/v2/projects/${projectId}/datasets/${datasetId}/tables`, undefined, qs);
    let { tables } = response;
    if (filter) {
