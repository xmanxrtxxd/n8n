                ...
                routing: {
                    ...
                    output: {
                        postReceive: [
                            GenericFunctions_1.handleErrorPostReceive,
                            async function (_data, response) {
                                const { metadata, ...properties } = (0, GenericFunctions_1.parseHeaders)(response.headers);
                    ...
                    output: {
                        postReceive: [
                            ...
                            async function (data, _response) {
                                return [
                                    {
                                        json: await (0, GenericFunctions_1.parseContainerList)(data[0].json),
                                    },
                                ];
                            },
                        ],
                ...
                preSend: [
                    async function (requestOptions) {
                        ...
                        if (container.length < 3 || container.length > 63) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), "'Container Name' must be from 3 through 63 characters long");
                        }
                        if (/[A-Z]/.test(container)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), "All letters  in 'Container Name' must be lowercase");
                        }
                        if (!/^[a-z0-9-]+$/.test(container)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), "'Container Name' can contain only letters, numbers, and the hyphen/minus (-) character");
                        }
                        if (!/^[a-z0-9].*[a-z0-9]$/.test(container)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), "'Container Name' must start or end with a letter or number");
                        }
                        if (/--/.test(container)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), "Consecutive hyphens are not permitted in 'Container Name'");
                        }
        ...
        options: [
            {
                ...
                routing: {
                    request: {
                        headers: {
                            [GenericFunctions_1.HeaderConstants.X_MS_BLOB_PUBLIC_ACCESS]: '={{ $value || undefined }}',
                        },
                    },
                },
                type: 'options',
            ...
            operations: {
                async pagination(requestOptions) {
                    ...
                    do {
                        ...
                        executions = executions.concat(responseData[0].json.containers.map((item) => ({ json: item })));
                    } while (marker);
                    return executions;
                },
            },
